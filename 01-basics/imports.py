...
dt_string = cas.strftime("%d/%m/%Y %H:%M:%S")

print("Prave ted je:", dt_string)
...
print("Nejbližší Štědrý den v neděli je v roce %s" % year)

for i in range(-1,4):
    rdelta = relativedelta(easter(year+i), today)
    print("velikonoční neděle: %s" % (today+rdelta))
""""""